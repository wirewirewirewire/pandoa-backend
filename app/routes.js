...
module.exports = function(app, passport) {
      ...
      newCase.save(function(err) {
        if (err) {
          ...
          for (var k in req.body.data) {
            let speed = req.body.data[k].speed
              ? req.body.data[k].speed
              : undefined;
            let geocode = req.body.data[k].geocode
              ? req.body.data[k].geocode[0]
              : undefined;
  ...
  app.get("/api/v1/download", function(req, res) {
    if (req.query.startId && ObjectId.isValid(req.query.startId) != true) {
      return res.status(403).send({
        success: false,
        _error:
          "Error: ObjectID emty format. Argument passed in must be a single String of 12 bytes or a string of 24 hex characters."
      });
    }
    var limit = 5000;
    var d = new Date();
    ...
    if (req.query.startId)
      search["_id"] = { $gte: ObjectId(req.query.startId) };
    Store.aggregate(
      [
        {
          $lookup: {
      ...
      function(err, result) {
        if (result.length <= 0)
          return res.send({ success: false, _error: "No Stores found" });
        var latest = result.slice(-1).pop()._id;
        var is_update = result.length < limit;
        var f = new Date();
        var diff = Math.abs(d - f);
        console.log(
          "[Router] Case Download ID: " +
            req.query.startId +
            " Points: " +
            result.length
        );
  ...
  app.get(
    "/api/v1/case/del",
    connectEnsureLogin.ensureLoggedIn("/api/v1/login"),
    function(req, res) {
      ...
      Case.findOne(search_cases, {}, function(err, result) {
        ...
        if (result) {
          ...
          return res
            .status(403)
            .send({ success: false, _error: "No Case found to Delete" });
        }
      });
    }
  );
